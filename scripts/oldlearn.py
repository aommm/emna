...
def load_features():
  ...
  rows = cur.fetchall()
  # Create dict
  featuresByScheme = dict()
  for [lemma, feature, scheme] in rows:
    if not scheme in featuresByScheme:
      featuresByScheme[scheme] = dict()
    if not lemma in featuresByScheme[scheme]:
      featuresByScheme[scheme][lemma] = dict()
    featuresByScheme[scheme][lemma][feature] = 1

  return featuresByScheme

def get_features_of_schemes(featuresByScheme, schemes):
  features = dict()

  for s in schemes:
    lemmas = featuresByScheme[s]
    for lemma,feats in lemmas.iteritems():
      features[lemma] = feats
